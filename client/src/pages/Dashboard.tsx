    ...
    <AppShell title="Overview">
      {showOnboarding && (
        <Card
          ...
          className="mb-6 overflow-hidden rounded border-0 bg-gradient-to-br from-[#475C65] to-[#3d5059] shadow-[0px_4px_6px_-4px_#0000001a,0px_10px_15px_-3px_#0000001a]"
        >
          <CardContent className="p-6 md:p-8">
            <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
              <div className="flex items-start gap-4">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded bg-white/15 text-white">
              ...
              <div className="flex flex-col gap-2 sm:flex-row md:shrink-0">
                <Link href="/settings">
                  <Button
                    ...
                    variant="outline"
                    className="h-11 w-full gap-2 rounded border-white/40 bg-transparent text-white hover:bg-white/10 hover:text-white sm:w-auto"
                  >
                ...
                <Link href="/log">
                  <Button
                    ...
                    className="h-11 w-full gap-2 rounded bg-white text-base font-bold text-[#475C65] hover:bg-white/90 sm:w-auto"
                  >
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.6fr)_minmax(320px,418px)]">
          ...
          <CardContent className="flex h-full flex-col p-6 md:p-8">
            <div className="flex flex-col justify-between gap-6 md:flex-row">
              <div className="flex-1">
                <p className="text-xs font-bold uppercase leading-4 tracking-[1.2px] text-[#475C65]">Current Status</p>
                <div className="mt-3 flex items-end gap-3">
                  <span data-testid="text-today-calories" className="text-5xl font-bold leading-[56px] tracking-[-0.96px] text-[#475C65]">
                    {totals.calories}
                  </span>
                  <span className="pb-2 text-xl font-normal text-[#475C65]">kcal consumed</span>
                </div>
                <p className="mt-4 text-base text-[#424843]">
                  You have <span className="font-bold text-[#475C65]">{remaining} kcal</span> remaining to reach your daily goal of {goal} kcal.
                </p>
              ...
              <div className="flex justify-center xl:justify-end">
                <div
                  ...
                  style={{
                    background: `conic-gradient(#475C65 ${pct * 3.6}deg, #e7e5df 0deg)`,
                  }}
                >
                  <div className="flex h-[80%] w-[80%] flex-col items-center justify-center rounded-full bg-white">
                    <div className="text-3xl font-bold text-[#475C65]" data-testid="text-goal-percent">{pct}%</div>
        ...
        <div className="flex flex-col gap-6">
          <Card className="overflow-hidden rounded border-0 bg-[#475C65] shadow-[0px_4px_6px_-4px_#0000001a,0px_10px_15px_-3px_#0000001a]">
                  ...
                  weightDeltas.map((item, i) => (
                    <div key={item.week}>
                      <div className="flex items-center justify-between py-3">
                        ...
                        <span className="text-base font-bold text-[#475C65]">
        ...
        <CardContent className="p-6 md:p-8">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-6">
            <div className="flex items-center gap-2">
              <span className="h-3 w-3 rounded-full bg-[#475C65]" />
              <p className="text-xs text-[#424843]">Daily Average: <span className="font-bold">{avg.toLocaleString()} kcal</span></p>
            </div>
            <div className="flex items-center gap-2">
              <span className="h-3 w-3 rounded-full border border-[#475C6533] bg-[#8aaab3]" />
